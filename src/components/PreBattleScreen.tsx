/**
 * 팔자전 — 출전준비 화면 (PreBattleScreen)
 * Phase 1.6 C항목: 패시브 선택 흐름 통합
 *
 * 3단계 카드 형식:
 *  1️⃣ 오늘의 패 확인 (현재 손 3장 표시, 건너뛰기)
 *  2️⃣ 가호 선택 (무작위 4개 → 2개 선택)
 *  3️⃣ 전투 시작 버튼
 */
...
// 간단한 LCG — 4개 패시브 추출
function pickRandom4(seed: number): Passive[] {
  let s = seed
  const rng = () => {
...
export default function PreBattleScreen({ hand, onComplete, seed }: PreBattleScreenProps) {
  const [step, setStep] = useState<1 | 2 | 3>(1)
  const [selectedPassives, setSelectedPassives] = useState<string[]>([])

  const passiveOptions = useMemo(
    () => pickRandom4(seed ?? Date.now()),
  ...
  const handleConfirmPassives = () => {
    if (selectedPassives.length < 2) {
      // 2개 미선택 시 자동으로 앞 두 개 선택
      const auto = passiveOptions.slice(0, 2).map(p => p.id)
  ...
  const handleSkipPassives = () => {
    // 건너뛰기 → 기본 패시브 자동 장착
    const auto = passiveOptions.slice(0, 2)
    onComplete(auto)
  }

  const handleStart = () => {
    const chosen = passiveOptions.filter(p => selectedPassives.includes(p.id))
    // 2개 미만이면 앞에서 채움
    const result =
      chosen.length >= 2
        ? chosen.slice(0, 2)
        : [...chosen, ...passiveOptions.filter(p => !selectedPassives.includes(p.id))].slice(0, 2)
    onComplete(result)
  }
      ...
      >
        {([1, 2, 3] as const).map(s => (
          ...
          >
            ...
            {s < 3 && (
              <span style={{ color: '#2A2620', fontSize: '12px' }}>→</span>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 16px' }}>
              ...
              {displayHand.length === 0 ? (
                ...
                >
                  <div
                    style={{
                      fontSize: '22px',
                      color: ELEMENT_COLORS[card.element] ?? '#D8CCB4',
                      fontWeight: 'bold',
                    }}
                  >
                ...
                return (
                  ...
                  >
                    <div
                      style={{
                        fontSize: '9px',
                        color: PASSIVE_RARITY_COLORS[passive.rarity],
                        letterSpacing: '0.1em',
                        marginBottom: '5px',
                      }}
                    >
        ...
        {step === 3 && (
          <div style={{ width: '100%', maxWidth: '380px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '28px' }}>
              {passiveOptions
                .filter(p => selectedPassives.includes(p.id))
                .map(passive => (
                  <div
                    key={passive.id}
                    style={{
