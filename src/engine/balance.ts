 ...
 * 규칙: 같은 전투에서 gather5(또는 5장 모으기) 반복 발동 시
 *      1회차 ×6.5 → 2회차 ×5.0 → 3회차+ ×4.0
 * @param gatherUsedInBattle  현재 전투에서 gather 발동 횟수 (1, 2, 3+)
 * @returns 적용할 배율
 */
export function getGather5Multiplier(gatherUsedInBattle: number): number {
  if (gatherUsedInBattle <= 0) return RECIPE_GATHER5_MULT_A  // 7.5 — 1회차 (§1 실효 복원 2026-07-18)
  if (gatherUsedInBattle === 1) return 6.5  // 2회차 (§1 실효 복원 2026-07-18)
  return 6.0  // 3회차+ (§1 실효 복원 2026-07-18)
}
...
export const RECIPE_GATHER5_MULT_A = 7.5  // §1 실효 복원 (구 6.5 → 7.5, 조화 +20% 폐지 보상)
...
/** 역생: 적이 나를 생 → ×1.0 (이든 확정 2026-07-19, 중립 — 하늘이 보태주지도 막지도 않는다) */
export const YIKSEANG_MULT = 1.0
...
// --- 배치 2 §2: 왕·여왕 카드
export const ROYAL_DECK_CAP = 2  // 왕+여왕 합산 덱 소지 상한

export interface RoyalCardDef {
  id: string
  name: string           // 병기: "병화 丙火 · 불의 왕"
  element: Element
  polarity: Polarity     // 왕=yang, 여왕=yin
  royalType: 'king' | 'queen'
  cheonganName: string   // 천간명 (갑~계)
}

export const ROYAL_CARDS: RoyalCardDef[] = [
  // 왕 5종 (양간)
  { id: 'king-hwa',  name: '병화 丙火 · 불의 왕',  element: 'hwa',  polarity: 'yang', royalType: 'king',  cheonganName: '병화' },
  { id: 'king-mok',  name: '갑목 甲木 · 나무의 왕', element: 'mok',  polarity: 'yang', royalType: 'king',  cheonganName: '갑목' },
  { id: 'king-to',   name: '무토 戊土 · 흙의 왕',   element: 'to',   polarity: 'yang', royalType: 'king',  cheonganName: '무토' },
  { id: 'king-geum', name: '경금 庚金 · 쇠의 왕',   element: 'geum', polarity: 'yang', royalType: 'king',  cheonganName: '경금' },
  { id: 'king-su',   name: '임수 壬水 · 물의 왕',   element: 'su',   polarity: 'yang', royalType: 'king',  cheonganName: '임수' },
  // 여왕 5종 (음간)
  { id: 'queen-hwa',  name: '정화 丁火 · 불의 여왕',  element: 'hwa',  polarity: 'yin', royalType: 'queen', cheonganName: '정화' },
  { id: 'queen-mok',  name: '을목 乙木 · 나무의 여왕', element: 'mok',  polarity: 'yin', royalType: 'queen', cheonganName: '을목' },
  { id: 'queen-to',   name: '기토 己土 · 흙의 여왕',   element: 'to',   polarity: 'yin', royalType: 'queen', cheonganName: '기토' },
  { id: 'queen-geum', name: '신금 辛金 · 쇠의 여왕',   element: 'geum', polarity: 'yin', royalType: 'queen', cheonganName: '신금' },
  { id: 'queen-su',   name: '계수 癸水 · 물의 여왕',   element: 'su',   polarity: 'yin', royalType: 'queen', cheonganName: '계수' },
]

/** 왕족 카드 생성 (층 보상·뽑기 공용) */
export function createRoyalCard(def: RoyalCardDef, value: number, uniqueSuffix: string): Card {
  return {
    id: `${def.id}-${uniqueSuffix}`,
    element: def.element,
    ...
    rarity: 'epic',
    name: def.name,
    royalType: def.royalType,
  }
}

/** 현재 덱에서 왕족 카드 수 카운트 */
export function countRoyalCards(cards: Card[]): number {
  return cards.filter(c => c.royalType === 'king' || c.royalType === 'queen').length
}
 ...
 *  - 2층: 445 × 1.40 = 623
 *  - 3층: 680 × 1.40 = 952
 *  - 4층: 680 (구 v3 원값 복귀 — ×1.15 하향은 경사 전 세계의 유산.
 *    제라 스윕 ZERA_V4_TODANIL_HP_SWEEP_20260718: 목화28.4/금수34.6/토단일34.8,
 *    격차6.4, 목화·금수 무풍 ±2%p 준수. 730안 기각(금수 −3.9 무풍 위반). 이든 확정 2026-07-18)
 */
export const V4_FLOOR_HP_TABLE: Record<number, number> = {
  1: Math.round(220 * 1.60),  // 352 (×1.60 — 2026-07-22 이든 확정, 3벌 측정 밴드 안착(목화20.0/금수26.9/토단일20.7))
  2: Math.round(445 * 1.60),  // 712 (×1.60)
  3: Math.round(680 * 1.60),  // 1088 (×1.60)
  4: 680,                      // 불변 확정 (4층은 극상성 사망층, HP 조정 금지)
}
