/**
 * 강림제 §4-b — B-1 "대기창" (재구현, 2026-07-15)
 *
 * 정의: 슬롯 도래 → 최대 3공격 대기. 창 내 용신 포함 콤보 → ×2.0 발동 + 창 닫힘.
 *       3공격 경과 → 소멸. 카운터제 아님 — 슬롯당 1회, 대기는 슬롯 귀속.
 *
 * 판정 기준 (§4-b 개정):
...
vi.mock('../engine/balance', async () => {
  ...
  return {
    ...
    DESCENT_VARIANT: 'wait3' as const,
  }
})
...
describe('강림제 §4-b — B-1 "대기창" (슬롯 귀속, 3공격 대기)', () => {
  it(
    ...
    () => {
      ...
      console.log('\n' + '='.repeat(110))
      console.log('강림제 §4-b — B-1 "대기창" × 1000판 × 3프리셋')
      console.log('판정: 포착률(목화·금수 ≥70%, 토단일 ≥40%) + 소멸률 <30% + 클리어율 25~40')
      console.log('E2E 지문: descentState.slots.includes(currentTurn)')
