/**
 * v4 희소성 복원 — B벌 측정 (수정판: getFloorHp 직접 mock 주입)
 *
 * B벌: { peak:1.0, step1:0.75, step2:0.50 }
 * v4 HP: getFloorHp 직접 mock → 308/623/952/621
 * YEONHWAN_MIN_SUM=25: 코드 실제값 (pokerHandJudge.ts — mock 없음)
 *
 * 산출물: /tmp/v4_sparsity_B.json
 */
...
const TABLE = { peak: 1.0, step1: 0.75, step2: 0.50 }

describe('v4 희소성 복원 — B벌 측정 (getFloorHp 직접 mock)', () => {
  it(
    'B벌 1000판 × 3프리셋 측정',
    { timeout: 900000 },
    () => {
      ...
      for (const preset of PRESETS) {
        ...
        console.log(
          `[B벌] ${preset.label}: 클리어율 ${clearRate.toFixed(1)}% (${victories}/${RUNS}) — ${gatePass ? 'PASS' : 'FAIL'}`,
        )
      }
      ...
      console.log(`\n[B벌] 격차: ${spread.toFixed(1)}%p — ${spreadPass ? 'PASS' : 'FAIL'}`)
      console.log(`[B벌] 게이트: ${allPass && spreadPass ? 'PASS' : 'FAIL'}`)
      ...
      console.log(`\n[B벌] 정점 발동: ${meanStep0.toFixed(2)} ± ${sigmaStep0.toFixed(2)} 회/판`)
      console.log(`[B벌] 비정점 발동: ${meanNonPeak.toFixed(2)} ± ${sigmaNonPeak.toFixed(2)} 회/판`)
      console.log(`[B벌] 비정점 평균 배율: ×${avgNonPeakMult.toFixed(3)}`)
      console.log(`[B벌] 5장 발동률: ${fusion5Rate.toFixed(1)}%`)
      console.log(`[B벌] 연환 성립률: ${yeonhwanRate.toFixed(2)}%`)
      ...
      writeFileSync('/tmp/v4_sparsity_B.json', JSON.stringify(output, null, 2))
      console.log('\n[B벌] → /tmp/v4_sparsity_B.json 저장 완료')
