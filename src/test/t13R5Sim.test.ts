/**
 * T13 R5 본시뮬 — 3000판 (2026-07-13 이든 지시)
 *
 * HANDOFF §4 완전 준수:
 *  1. 커밋 해시 = b400907 (sikshin D안 구현)
 *  2. selectTalismanBySaju 코드 라인 = fullCapBot.ts:L942
 *  3. enableFloorReward = true (opts.enableFloorReward: true 전달 L616)
 *  4. 표준 3종 프리셋:
 *     - 목화: {mok:4, hwa:4, to:2, geum:2, su:2}
 *     - 금수: {mok:2, hwa:2, to:2, geum:4, su:4}
 *     - 토단일: {mok:1, hwa:1, to:14, geum:2, su:2}
 *  5. 시드 식 = i * 12345 + 7777
 *  6. 목화 프리셋 → selectTalismanBySaju 결과: 실측 (테스트 내 로그)
 *
 * 가호 장착 방식: selectTalismanBySaju(dist) 사주 기반 (a)안 — 고정 지정 금지
 * enableFloorReward: true (생략 불가)
 * 판 수: 3000판
 *
 * 실행: npm test -- src/test/t13R5Sim.test.ts
 */
...
describe('T13 R5 본시뮬 — 사주기반 가호 3000판 (sikshin D안 포함)', () => {
    ...
    () => {
      ...
      for (const preset of PRESETS) {
        ...
        for (let i = 0; i < RUNS; i++) {
          ...
          // 상관 발동 추적
          sanggwanTotal += result.traitCounts['passive_sanggwan'] ?? 0
      ...
      console.log('\n========================================')
      console.log('T13 R5 본시뮬 보고서 (3000판)')
      console.log('커밋: b400907 | 시드: i*12345+7777 | enableFloorReward: true')
      ...
      // 가호 발동 통계
      console.log('\n## 가호 발동 통계')
      console.log('| 프리셋 | 선택 가호 | 상관 발동/판 | sikshin D안 버리기/판 | sikshin 낱장/판 |')
      console.log('|--------|----------|------------|---------------------|----------------|')
      for (const r of results) {
