/**
 * v4 희소성 복원 — A벌 측정 (2026-07-18 이든 지시)
 *
 * A벌: V4_RATIO_CORRECTION = { peak:1.0, step1:0.70, step2:0.45 }
...
// ─── V4_RATIO_CORRECTION A벌 교체 + v4 HP 주입 ────────────────────────────
...
describe('v4 희소성 복원 — A벌 측정 (step1=0.70/step2=0.45)', () => {
  it(
    'A벌 1000판 × 3프리셋 + 정점/비정점 하드 넘버 + 5장 발동률 + 연환 성립률',
    { timeout: 900000 },
    () => {
      // HP 확인
      console.log(`\n[A벌] V4_FLOOR_HP_TABLE: 1층=${V4_FLOOR_HP_TABLE[1]} / 2층=${V4_FLOOR_HP_TABLE[2]} / 3층=${V4_FLOOR_HP_TABLE[3]} / 4층=${V4_FLOOR_HP_TABLE[4]}`)
      console.log(`[A벌] v4 FLOOR_CONFIGS HP: ${FLOOR_CONFIGS.map((c: { floor: number; enemyHp: number }) => `${c.floor}층=${c.enemyHp}`).join(' / ')}`)

      // 집계 변수
      let totalStep0 = 0   // 정점 (N≥3, steps=0)
      let totalStep1 = 0   // 한 계단
      let totalStep2 = 0   // 두 계단
      let total2CardExempt = 0  // 2장 면제
      let total5Card = 0   // 대융합 5장
      let totalYeonhwan = 0    // 연환 발동 횟수
      let totalFusionAll = 0   // 전체 융합 (step0+step1+step2+2장면제)
      let totalAttacks = 0     // 전체 공격 턴
      ...
      for (const preset of PRESETS) {
        ...
        for (let i = 0; i < RUNS; i++) {
          ...
          // 전체 공격 턴 (모든 comboResult — fusionCount + gather + yeonhwan + none 합산)
          // fusionCount = 전체 융합 횟수
          totalFusionAll += result.fusionCount ?? 0

          // 전체 공격 턴 = floorStats attackCount 합산
        ...
        console.log(
          `[A벌] ${preset.label}: 클리어율 ${clearRate.toFixed(1)}% (${victories}/${RUNS}) — 게이트 ${gatePass ? 'PASS' : 'FAIL'}`,
        )
      }
      ...
      // 정점/비정점 분포 하드 넘버
      const totalFusionHG = totalStep0 + totalStep1 + totalStep2  // N≥3 융합 합계
      const totalFusionIncl2 = totalFusionHG + total2CardExempt    // 전체 융합

      console.log('\n=== A벌 정점/비정점 실발동 분포 (하드 넘버) ===')
      ...
      const output = {
        variant: 'A',
        table: { peak: 1.0, step1: 0.70, step2: 0.45 },
        presets: results,
      ...
      writeFileSync('/tmp/v4_scarcity_A.json', JSON.stringify(output, null, 2))
      console.log('\n[A벌] 결과 → /tmp/v4_scarcity_A.json 저장 완료')
