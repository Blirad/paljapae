...
describe.skip('1. 양자택일 분기 — 들불(mok+hwa, fusion-birth)', () => {
...
describe.skip('2. 잔불(wildfire) — 들불 + effectMode=true', () => {
...
describe.skip('3. 자양(nourish) — 숲(su+mok) + effectMode=true', () => {
...
describe.skip('4. 정화(purification) — 샘(geum+su) + effectMode=true', () => {
...
describe.skip('5. 채굴(mining) — 광맥(to+geum) + effectMode=true', () => {
...
describe.skip('6. 수렴 불변식 — 채굴 드로우 후 손패 ≤ HAND_SIZE+MINING_MAX_DRAW', () => {
...
describe.skip('7. 버리기 3장 제한 — discardCards에 4장 전달 시 throw (명시적 거부)', () => {
  it('4장 버리기 시도 → throw (MAX_DISCARD_PER_USE=3 초과)', () => {
    ...
    // 8e444af clamp 회귀 근절 — throw로 명시적 거부
...
describe.skip('8. 모으기 4장 배율 — 같은 기운 4장 → GATHER_MULTIPLIERS[4]=4.0', () => {
